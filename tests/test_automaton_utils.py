...
def test_bfs_rpq_5() -> None:
    ...
    regex = Regex("")
    regex1 = Regex("$")
    assert 11 == len(utils.bfs_rpq(graph, regex)) and utils.bfs_rpq(graph, regex) == utils.bfs_rpq(graph, regex1)


def test_bfs_rpq_6() -> None:
    graph = graph_utils.get_labeled_two_cycles_graph(5, 5, labels=("a", "b"))
    regex = Regex("a")
    assert {4} == utils.bfs_rpq(graph, regex, start_states={3}) and {0} == utils.bfs_rpq(graph, regex, start_states={5})
...
def test_bfs_rpq_7() -> None:
    ...
    assert expected == utils.bfs_rpq(graph, regex, start_states={3}) and expected == utils.bfs_rpq(graph, regex, start_states={0})
...
def test_bfs_rpq_8() -> None:
    ...
    regex = Regex("g")
    assert {(88, 89)} == utils.bfs_rpq(graph, regex, start_states={88}, foreach_start_node = True)


def test_bfs_rpq_9() -> None:
    graph = graph_utils.get_labeled_two_cycles_graph(50, 50, labels=("g", "h"))
    regex = Regex("g+(h*)")
    assert {(10, 11)} == utils.bfs_rpq(graph, regex, start_states={10}, foreach_start_node = True)
    regex1 = Regex("(g*)+h")
    assert {(10, x) for x in range (0, 51)} == (utils.bfs_rpq(graph, regex1, start_states={10}, foreach_start_node = True)) 
    assert {(10, 11)} == (utils.bfs_rpq(graph, regex1, start_states={10}, final_states={11}, foreach_start_node = True))
    assert {(x, 11) for x in range (0, 51)} == (utils.bfs_rpq(graph, regex1, final_states={11}, foreach_start_node = True)) 
